...
| **ChatGPT Web / Gemini Web / Manus API** | ✅ | 同样通过工具说明 + 多轮上下文 + `<tool_call>` 解析实现（详见 [工具调用说明](docs/TOOL_CALLING_MODELS.md)）。 |
| **OpenRouter / OpenAI 兼容 API** | ✅ | 使用原生 `tools` / `tool_calls` 接口。 |
| **Ollama** | ✅ | 使用原生 `/api/chat` 的 tools。 |

Agent 的文件访问范围受配置中的**工作区**目录限制（见配置项 `agents.defaults.workspace`）。详细说明与验证步骤见 **[docs/TOOL_CALLING_MODELS.md](docs/TOOL_CALLING_MODELS.md)**。
